...
export async function handleAIGame(socket, request) {
    ('AI Game WebSocket connection established');
    ...
    async function updateBall() {
        ...
        if (gameState.playerScore >= gameState.winningScore || gameState.aiScore >= gameState.winningScore) {
            ...
            ('AI game finished - result will be saved by frontend');
    ...
    // Start the game
    function startGame() {
        ('🎯 startGame() called, gameState.gameStarted:', gameState.gameStarted);
        if (gameState.gameStarted) {
            ('⚠️ Game already started, returning');
            return;
        }
        
        ('✅ Starting new AI game...');
        ...
        ('📤 Sending game-started message to frontend');
        socket.send(JSON.stringify({
            type: 'game-started',
    ...
    socket.onmessage = (event) => {
        try {
            ...
            ('🎮 AI Game received message:', data.type, data);
    ...
    socket.onclose = () => {
        ('AI Game WebSocket connection closed');
        stopGame();
    };
