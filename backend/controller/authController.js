...
export async function login(req, reply) {
    ...
    ('Request body:', { email, hasPassword: !!password, twoFactorCode });
    ...
    ('✅ Email/password valid. User 2FA status:', user.isTwoFactorEnabled);

    // Check if 2FA is enabled
    if (user.isTwoFactorEnabled) {
        if (!twoFactorCode) {
            ('🔐 2FA required, no code provided - showing 2FA form');
        ...
        ('🔍 Verifying 2FA code:', twoFactorCode);
        ('🔑 User 2FA secret exists:', !!user.twoFactorSecret);
        ...
        if (user.twoFactorBackupCodes) {
            try {
                ...
                ('📱 Available backup codes count:', backupCodesArray.length);
            } catch (e) {
                ('⚠️ Error parsing backup codes:', e.message);
                backupCodesArray = [];
            }
        }
        ...
        ('🔐 TOTP verification result:', verified);
        ...
        if (!verified) {
            ('❌ 2FA verification failed for code:', twoFactorCode);
            ('Available backup codes:', backupCodesArray);
