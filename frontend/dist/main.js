...
class SimpleAuth {
    ...
    renderComprehensiveDashboardData(dashboardData) {
        if (!dashboardData)
            return;
    ...
    initializeAIGame() {
        ...
        if (typeof window !== 'undefined') {
            // Try direct access
            aiGameFunction = window.initializeAIGame;
            console.log('🎮 Direct function available:', typeof aiGameFunction);
            // Try through simpleAuth object
            if (!aiGameFunction && window.simpleAuth) {
                aiGameFunction = window.simpleAuth.initializeAIGame;
    ...
    renderHomeDashboardData(dashboardData) {
    ...
    async updateUserStats(userWon, gameType = 'LOCAL', player1Score, player2Score, opponentName) {
